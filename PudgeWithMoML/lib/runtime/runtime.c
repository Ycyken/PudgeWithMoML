...
extern void *__start_global_vars __attribute__((weak));
extern void *__stop_global_vars __attribute__((weak));
extern void *__start_global_closures __attribute__((weak));
extern void *__stop_global_closures __attribute__((weak));
...
static void update_stack_data_ptrs(void *current_sp, void *old, void *new) {
  ...
  // Обновляем указатели в глобальных переменных
  for (void **p = &__start_global_vars; p < &__stop_global_vars; ++p) {
    if (*p == old)
      *p = new;
  }

  // Обновляем указатели в глобальных замыканиях
  for (void **p = &__start_global_closures; p < &__stop_global_closures; ++p) {
    if (*p == old)
      *p = new;
  }
...
static void _gc_collect(void *current_sp) {
  ...
  while (cur_offset < gc.alloc_offset) {
    ...
    // try to find in stack at least one pointer
    {
      ...
      // Проверяем глобальные замыкания (если не нашли в переменных)
      if (!found) {
        for (void **p = &__start_global_closures; p < &__stop_global_closures; ++p) {
          if (*p == cur_pointer) {
            found = true;
...
void *apply_closure(INT8, closure *old_clos, uint8_t argc, ...) {
  ...
  void *result = _apply_closure(old_clos, argc, new_args);
  free(new_args);
  return result;
}
